...
public class StorageClient {
	 ...
	 */
	/*
	 * public StorageClient() { this.trackerServer = null; this.storageServer =
	 * null; }
	 */
	...
	public StorageClient(TrackerServer trackerServer, TrackerClient trackClient) {
		this.trackerServer = trackerServer;
	...
	protected String[] do_upload_file(byte cmd, String group_name, String master_filename, String prefix_name,
		...
		/*
		 * 上传协议格式
		 * |heaer(pkg_ken(8)+cmd(1)+errno(1)|文件名长度（8）|文件大小（8）|文件前缀（6如果有）|扩展名（6不足用0填充）|
		 * 文件名（实际长度）|文件内容（实际长度）|
		 */
		byte[] header;
		byte[] ext_name_bs;
